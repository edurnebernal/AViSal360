...
def KLD(saliency_map1, saliency_map2):
	map1 = normalize(saliency_map1, method='sum') 
	map2= normalize(saliency_map2, method='sum') # gt fixation map
	return np.sum(np.where(map2 != 0, map2 * np.log((map2+EPSILON) / (map1+EPSILON)), 0))
...
def MSE(saliency_map1, saliency_map2):
    '''
    Mean Squared Error between two different saliency maps
    (MSE=0 for identical maps).
    ...
    if map1.shape != map2.shape:
        map1 = resize(map1, map2.shape, order=3, mode='nearest') # bi-cubic/nearest is what Matlab imresize() does by default
    # Normalize the two maps to have values between [0,1]
    map1 = normalize(map1, method='range')
    map2 = normalize(map2, method='range')
    # Compute mean squared error
    return np.mean((map1 - map2)**2)

def MAE(saliency_map1, saliency_map2):
    '''
    Mean Absolute Error between two different saliency maps
    (MAE=0 for identical maps).
...
def EMD(saliency_map1, saliency_map2, sub_sample=1/32.0):
    ...
    signature1 = np.column_stack((map1.ravel(), x.ravel(), y.ravel())).astype(np.float32)
    signature2 = np.column_stack((map2.ravel(), x.ravel(), y.ravel())).astype(np.float32)
    distance, lowerbound, flow_matrix = cv.EMD(signature1, signature2, cv.DIST_L2)
    return distance